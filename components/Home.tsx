...
import { Github, Linkedin, Mail, Code2, Monitor } from "lucide-react"
import Navbar from "@/components/ui/navbar"
import SectionAnimation from "@/components/ui/section-animation"
import { WindowsUI } from "@/components/windows-ui"
          ...
          <div className="container mx-auto relative z-[5] px-4 md:px-6 flex flex-col items-center">
            <div className="text-center mb-8">
              <h1 className="text-4xl md:text-6xl font-bold tracking-tighter mb-4 text-cyan-400">
                <span className="text-white">Hi! I'm </span> Bhagya Vishwakarma
              </h1>
            ...
            <div className="flex items-center justify-center gap-4 mt-4">
              <button
                ...
                className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition-colors border border-gray-700 hover:border-cyan-500"
              >
                {isDevMode ? (
                  <>
                    <Monitor className="w-5 h-5" />
                    <span className="font-mono text-sm">Switch to Simple Mode</span>
                  </>
                ) : (
                  <>
                    <Code2 className="w-5 h-5" />
                    <span className="font-mono text-sm">Switch to Dev Mode</span>
                  </>
                )}
              </button>
            </div>
              ...
              {isDevMode ? (
                <>
                  ...
                  <div className="flex flex-wrap justify-center gap-4">
                    <button 
                      onClick={() => handleOpenProject('projects')}
                      className="bg-cyan-700 hover:bg-cyan-600 text-white p-2"
                    >
                      View Projects
                    </button>
                    <button 
                      onClick={() => handleOpenContact()}
                      className="border border-cyan-700 text-cyan-400 p-2 hover:bg-cyan-950"
                    >
                      Contact Me
                    </button>
                  </div>
                ...
                <p className="text-gray-400 mb-6">
                  Click on the icons above to explore my portfolio
                </p>
              )}
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-4 md:mb-0">
              <p className="text-gray-400">© {new Date().getFullYear()} Bhagya Vishwakarma. All rights reserved.</p>
            </div>
